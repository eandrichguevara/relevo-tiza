...
function createMockRequest(body: unknown): NextRequest {
  const url = 'http://localhost:3001/api/auth/set-token';
...
describe('POST /api/auth/set-token (tiza-web)', () => {
  beforeEach(() => {
  ...
  it('establece cookie HttpOnly con el token y maxAge de 24h', async () => {
    ...
    const authCookie = cookies.get('tiza-auth-token');
    ...
    expect(authCookie!.maxAge).toBe(86400); // 24h — COOKIE_MAX_AGE
  });
  ...
  it('establece secure=true cuando NODE_ENV es production', async () => {
    ...
    const authCookie = response.cookies.get('tiza-auth-token');
  ...
  describe('limpiar cookie con token vacío (logout)', () => {
    it('limpia la cookie cuando token es string vacío', async () => {
      ...
      expect(authCookie!.maxAge).toBe(0); // maxAge: 0 = eliminar cookie
      expect(authCookie!.httpOnly).toBe(true);
      expect(authCookie!.sameSite).toBe('strict');
      expect(authCookie!.path).toBe('/');
    });
