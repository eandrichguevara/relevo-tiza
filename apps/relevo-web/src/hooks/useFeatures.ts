 ...
 * con el header X-Tenant-Brand: relevo (incluido automáticamente por apiFetch).
 *
...
export function useFeatures() {
  ...
  return {
    features: data?.features ?? (isError ? getFeaturesMap('relevo') : ({} as FeatureFlagsMap)),
    brand: (data?.brand ?? 'relevo') as Brand,
    isLoading,
